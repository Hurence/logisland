...
import com.hurence.logisland.hbase.PutHBaseJSON;
...
public class TestPutHBaseJSON {
    ...
    public void testSingleJsonDocAndProvidedRowId() throws IOException, InitializationException {
        ...
        final MockRecord outFile = runner.getFlowFilesForRelationship(PutHBaseCell.REL_SUCCESS).get(0);
        outFile.assertContentEquals(content);

        assertNotNull(hBaseClient.getRecordPuts());
        assertEquals(1, hBaseClient.getRecordPuts().size());

        final List<PutRecord> puts = hBaseClient.getRecordPuts().get(DEFAULT_TABLE_NAME);
    ...
    public void testSingJsonDocAndExtractedRowIdMissingField() throws IOException, InitializationException {
        ...
        final MockRecord outFile = runner.getFlowFilesForRelationship(PutHBaseCell.REL_FAILURE).get(0);
        outFile.assertContentEquals(content);
        ...
        assertEquals(0, hBaseClient.getRecordPuts().size());
    }
    ...
    public void testELWithProvidedRowId() throws IOException, InitializationException {
        ...
        final List<PutRecord> puts = hBaseClient.getRecordPuts().get("myTable");
        assertEquals(1, puts.size());
