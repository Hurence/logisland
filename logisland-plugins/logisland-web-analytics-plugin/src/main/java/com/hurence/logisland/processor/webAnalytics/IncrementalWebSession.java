       ...
       extends AbstractElasticsearchProcessor
{
    ...
    private class Execution
    {
                ...
                implements Comparable<WebSession>
        {
            ...
            public void add(final WebEvent event)
            {
                ...
                if ( (_FIELDS_TO_RETURN != null) && (!_FIELDS_TO_RETURN.isEmpty()) )
                {
                    ...
                    {
                        field = event.record.getField(fieldnameToAdd);
                        if ( isFieldAssigned(field) )
                        {
                            record.setField(field.getName(), FieldType.STRING, field.asString());
                        }
                    }
                }
            ...
            {
                return fromEpoch(record.getField(_FIRST_EVENT_EPOCH_FIELD).asLong()*1000);
            }
            ...
            {
                return fromEpoch(record.getField(_LAST_EVENT_EPOCH_FIELD).asLong()*1000);
            }
        ...
        private class RecordItem
        {
            ...
            /**
             * Returns a copy of the inner record omitting {@code null} values.
             *
             * @return a copy of the inner record.
             */
            public Record cloneRecord()
            {
                ...
                this.record.getFieldsEntrySet()
                           .stream()
                           .forEach(entry -> result.setField(entry.getValue()));
