...
package com.hurence.logisland.plugin.generic
...
/**
  * This event mapper manages
  */
class GenericEventMapper extends EventMapper{

    val EVENT_TYPE = "event"
    override def getMapping: XContentBuilder = {
        jsonBuilder().startObject().startObject(EVENT_TYPE)
            .startObject("_ttl").field("enabled", "true").field("default", "700d").endObject()
