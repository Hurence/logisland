...
public class LogislandSessionConfigReader {
    ...
    public LogislandSessionConfiguration loadConfig(String configFilePath){
        ObjectMapper mapper = new ObjectMapper(new YAMLFactory());

        LogislandSessionConfiguration config = null;
        try {
            File configFile = new File(configFilePath);

            config = mapper.readValue(configFile, LogislandSessionConfiguration.class);
        } catch (IOException e) {
            e.printStackTrace();
