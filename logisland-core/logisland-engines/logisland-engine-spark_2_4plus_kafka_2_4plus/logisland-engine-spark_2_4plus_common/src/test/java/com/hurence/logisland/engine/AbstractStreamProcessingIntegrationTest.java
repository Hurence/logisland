...
public abstract class AbstractStreamProcessingIntegrationTest {
    ...
    public void setUp() throws InterruptedException, IOException {
        ...
        kafkaZkClient = kafkaServer.zkClient();
        adminZkClient = new AdminZkClient(kafkaZkClient);

        // create topics
        if (!kafkaZkClient.topicExists(StreamProperties.DEFAULT_ERRORS_TOPIC().getValue()))
        ...
        if (!kafkaZkClient.topicExists(StreamProperties.DEFAULT_RECORDS_TOPIC().getValue()))
            adminZkClient.createTopic(StreamProperties.DEFAULT_RECORDS_TOPIC().getValue(), 1, 1, new Properties(), RackAwareMode.Disabled$.MODULE$);
        if (!kafkaZkClient.topicExists(StreamProperties.DEFAULT_RAW_TOPIC().getValue()))
            adminZkClient.createTopic(StreamProperties.DEFAULT_RAW_TOPIC().getValue(), 1, 1, new Properties(), RackAwareMode.Disabled$.MODULE$);
        if (!kafkaZkClient.topicExists(StreamProperties.DEFAULT_METRICS_TOPIC().getValue()))
            adminZkClient.createTopic(StreamProperties.DEFAULT_METRICS_TOPIC().getValue(), 1, 1, new Properties(), RackAwareMode.Disabled$.MODULE$);


        // deleting zookeeper information to make sure the consumer starts from the beginning
        adminZkClient.deleteTopic("/consumers/group0");
...
*/
    ...
    public void tearDown() throws NoSuchFieldException, IllegalAccessException, InterruptedException {
        ...
        if (kafkaZkClient.topicExists(StreamProperties.DEFAULT_ERRORS_TOPIC().getValue()))
            adminZkClient.deleteTopic(StreamProperties.DEFAULT_ERRORS_TOPIC().getValue());
        if (kafkaZkClient.topicExists(StreamProperties.DEFAULT_RECORDS_TOPIC().getValue()))
            adminZkClient.deleteTopic(StreamProperties.DEFAULT_RECORDS_TOPIC().getValue());
        if (kafkaZkClient.topicExists(StreamProperties.DEFAULT_RAW_TOPIC().getValue()))
            adminZkClient.deleteTopic(StreamProperties.DEFAULT_RAW_TOPIC().getValue());
        if (kafkaZkClient.topicExists(StreamProperties.DEFAULT_METRICS_TOPIC().getValue()))
            adminZkClient.deleteTopic(StreamProperties.DEFAULT_METRICS_TOPIC().getValue());
