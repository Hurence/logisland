...
public interface CompactionApi {



    default Router getCompactionRouter(Vertx vertx) {
        Router router = Router.router(vertx);
        router.get("/").handler(this::root);
        router.post("/search").handler(this::search);
        router.post("/compact").handler(this::compact);
        router.post("/query")
    ...
    void compact(RoutingContext routingContext);
