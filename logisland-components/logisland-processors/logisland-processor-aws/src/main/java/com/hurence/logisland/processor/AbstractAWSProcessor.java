...
import com.hurence.logisland.processor.credentials.provider.factory.CredentialPropertyDescriptors;
import com.hurence.logisland.processor.services.SSLContextService;
...
public abstract class AbstractAWSProcessor<ClientType extends AmazonWebServiceClient> extends AbstractProcessor {
    ...
    protected static final String DEFAULT_USER_AGENT = "Logisland";
    ...
    protected Collection<ValidationResult> customValidate(final ValidationContext validationContext) {
        ...
        final boolean accessKeySet = validationContext.getPropertyValue(ACCESS_KEY).isSet();
        final boolean secretKeySet = validationContext.getPropertyValue(SECRET_KEY).isSet();
        ...
        final boolean credentialsFileSet = validationContext.getPropertyValue(CREDENTIALS_FILE).isSet();
        ...
        final boolean proxyHostSet = validationContext.getPropertyValue(PROXY_HOST).isSet();
        final boolean proxyPortSet = validationContext.getPropertyValue(PROXY_HOST_PORT).isSet();
        ...
        final boolean proxyUserSet = validationContext.getPropertyValue(PROXY_USERNAME).isSet();
        final boolean proxyPwdSet = validationContext.getPropertyValue(PROXY_PASSWORD).isSet();
    ...
    protected ClientConfiguration createConfiguration(final ProcessContext context) {
        ...
        final int commsTimeout = context.getPropertyValue(TIMEOUT).asTimePeriod(TimeUnit.MILLISECONDS).intValue();
        config.setConnectionTimeout(commsTimeout);
        config.setSocketTimeout(commsTimeout);

        if(this.getSupportedPropertyDescriptors().contains(SSL_CONTEXT_SERVICE)) {
            final SSLContextService sslContextService = context.getPropertyValue(SSL_CONTEXT_SERVICE).asControllerService(SSLContextService.class);
        ...
        final ProxyConfiguration proxyConfig = ProxyConfiguration.getConfiguration(context, () -> {
            if (context.getPropertyValue(PROXY_HOST).isSet()) {
                final ProxyConfiguration componentProxyConfig = new ProxyConfiguration();
                String proxyHost = context.getPropertyValue(PROXY_HOST).asString();
                Integer proxyPort = context.getPropertyValue(PROXY_HOST_PORT).asInteger();
                String proxyUsername = context.getPropertyValue(PROXY_USERNAME).asString();
                String proxyPassword = context.getPropertyValue(PROXY_PASSWORD).asString();
    ...
    protected void initializeRegionAndEndpoint(ProcessContext context) {
        ...
        if (getSupportedPropertyDescriptors().contains(REGION)) {
            final String region = context.getPropertyValue(REGION).asString();
            if (region != null) {
        ...
        if (getSupportedPropertyDescriptors().contains(ENDPOINT_OVERRIDE)) {
            final String urlstr = StringUtils.trimToEmpty(context.getPropertyValue(ENDPOINT_OVERRIDE).asString());
    ...
    protected AWSCredentials getCredentials(final ProcessContext context) {
        final String accessKey = context.getPropertyValue(ACCESS_KEY).asString();
        final String secretKey = context.getPropertyValue(SECRET_KEY).asString();

        final String credentialsFile = context.getPropertyValue(CREDENTIALS_FILE).asString();
