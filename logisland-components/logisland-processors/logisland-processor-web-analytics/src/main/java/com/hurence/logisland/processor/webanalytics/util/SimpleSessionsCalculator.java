 ...
 */
public class SimpleSessionsCalculator {

    private static Logger logger = LoggerFactory.getLogger(SimpleSessionsCalculator.class);
    ...
    public SimpleSessionsCalculator(Collection<SessionCheck> checkers,
    ...
    public String getLastSessionId() {
        if (!this.calculatedSessions.isEmpty()) {
            return this.calculatedSessions.get(this.calculatedSessions.size() - 1).getSessionId();
        } else {
            logger.error("Invalid state: session container is empty. " +
     ...
     */
    public SimpleSessionsCalculator processEvents(final Events events) {
        ...
        WebSession session = WebSession.fromWebEvent(firstEvent,
                webSessionInternalFields,
                fieldsToCopyFromEventsToSessions);
        add(session, firstEvent);
