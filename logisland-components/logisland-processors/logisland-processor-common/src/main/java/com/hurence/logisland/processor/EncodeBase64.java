...
@Tags({"encode", "base64"})
@CapabilityDescription("Encodes fields to base64. The fields should be of type array of bytes")
@ExtraDetailFile("./details/common-processors/EncodeBase64-Detail.rst")
public class EncodeBase64 extends AbstractProcessor {

    static final PropertyDescriptor SOURCE_FIELDS = new PropertyDescriptor.Builder()
            .name("source.fields")
            .description("a comma separated list of fields corresponding to the fields to encode. Please note than the " +
                    "fields should be of type bytes")
            .required(true)
    ...
    static final PropertyDescriptor DESTINATION_FIELDS = new PropertyDescriptor.Builder()
            ...
            .description("a comma separated list of fields corresponding to the encoded content according to the " +
                    "fields provided as input. Those fields will be of type string")
            .required(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .addValidator(StandardValidators.COMMA_SEPARATED_LIST_VALIDATOR)
            .build();
            ...
            for (int i = 0; i < sourceFieldNames.size(); i++) {
                ...
                if (f != null) {
                    if (!(f.getType() == FieldType.BYTES || f.getType() == FieldType.NULL)) {
                        ...
                        if (content != null) {
                            try {
                                record.setStringField(destinationFieldNames.get(i), BinaryEncodingUtils.encode(content));
                            } catch (Exception e) {
