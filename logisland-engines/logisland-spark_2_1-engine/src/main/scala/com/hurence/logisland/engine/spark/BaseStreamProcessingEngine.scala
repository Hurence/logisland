...
object BaseStreamProcessingEngine {
...
abstract class BaseStreamProcessingEngine extends AbstractProcessingEngine {

    private lazy val conf = new SparkConf()


    private val logger = LoggerFactory.getLogger(classOf[BaseStreamProcessingEngine])


    override def getSupportedPropertyDescriptors: util.List[PropertyDescriptor] = {
        val descriptors: util.List[PropertyDescriptor] = new util.ArrayList[PropertyDescriptor]
        descriptors.add(BaseStreamProcessingEngine.SPARK_APP_NAME)
        descriptors.add(BaseStreamProcessingEngine.SPARK_MASTER)
        descriptors.add(BaseStreamProcessingEngine.SPARK_YARN_DEPLOYMODE)
        descriptors.add(BaseStreamProcessingEngine.SPARK_YARN_QUEUE)
        descriptors.add(BaseStreamProcessingEngine.SPARK_DRIVER_MEMORY)
        descriptors.add(BaseStreamProcessingEngine.SPARK_EXECUTOR_MEMORY)
        descriptors.add(BaseStreamProcessingEngine.SPARK_DRIVER_CORES)
        descriptors.add(BaseStreamProcessingEngine.SPARK_EXECUTOR_CORES)
        descriptors.add(BaseStreamProcessingEngine.SPARK_EXECUTOR_INSTANCES)
        descriptors.add(BaseStreamProcessingEngine.SPARK_SERIALIZER)
        descriptors.add(BaseStreamProcessingEngine.SPARK_STREAMING_BLOCK_INTERVAL)
        descriptors.add(BaseStreamProcessingEngine.SPARK_STREAMING_BATCH_DURATION)
        descriptors.add(BaseStreamProcessingEngine.SPARK_STREAMING_BACKPRESSURE_ENABLED)
        descriptors.add(BaseStreamProcessingEngine.SPARK_STREAMING_UNPERSIST)
        descriptors.add(BaseStreamProcessingEngine.SPARK_UI_PORT)
        descriptors.add(BaseStreamProcessingEngine.SPARK_STREAMING_TIMEOUT)
        descriptors.add(BaseStreamProcessingEngine.SPARK_STREAMING_UI_RETAINED_BATCHES)
        descriptors.add(BaseStreamProcessingEngine.SPARK_STREAMING_RECEIVER_WAL_ENABLE)
        ...
        descriptors.add(BaseStreamProcessingEngine.SPARK_YARN_MAX_EXECUTOR_FAILURES)
        descriptors.add(BaseStreamProcessingEngine.SPARK_YARN_EXECUTOR_FAILURES_VALIDITY_INTERVAL)
        descriptors.add(BaseStreamProcessingEngine.SPARK_TASK_MAX_FAILURES)
        descriptors.add(BaseStreamProcessingEngine.SPARK_MEMORY_FRACTION)
        descriptors.add(BaseStreamProcessingEngine.SPARK_MEMORY_STORAGE_FRACTION)
    ...
    /**
      ...
      * @param engineContext
      */
    final override def start(engineContext: EngineContext) = {
        logger.info("starting Spark Engine")
        val timeout = engineContext.getPropertyValue(BaseStreamProcessingEngine.SPARK_STREAMING_TIMEOUT).asInteger().intValue()
        val batchDuration = engineContext.getPropertyValue(BaseStreamProcessingEngine.SPARK_STREAMING_BATCH_DURATION).asInteger().intValue()
    ...
    final def createStreamingContext(engineContext: EngineContext): StreamingContext = {
        val sparkMaster = engineContext.getPropertyValue(BaseStreamProcessingEngine.SPARK_MASTER).asString
        ...
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_STREAMING_UI_RETAINED_BATCHES)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_STREAMING_RECEIVER_WAL_ENABLE)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_UI_PORT)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_STREAMING_UNPERSIST)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_STREAMING_BACKPRESSURE_ENABLED)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_STREAMING_BLOCK_INTERVAL)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_SERIALIZER)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_DRIVER_MEMORY)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_EXECUTOR_MEMORY)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_DRIVER_CORES)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_EXECUTOR_CORES)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_EXECUTOR_INSTANCES)

        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_YARN_MAX_APP_ATTEMPTS)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_YARN_AM_ATTEMPT_FAILURES_VALIDITY_INTERVAL)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_YARN_MAX_EXECUTOR_FAILURES)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_YARN_EXECUTOR_FAILURES_VALIDITY_INTERVAL)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_TASK_MAX_FAILURES)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_MEMORY_FRACTION)
        setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_MEMORY_STORAGE_FRACTION)
        ...
        if (sparkMaster startsWith "yarn") {
            ...
            setConfProperty(conf, engineContext, BaseStreamProcessingEngine.SPARK_YARN_QUEUE)
        }

        customizeSparkConfiguration(conf, engineContext)
        ...
        val ssc = getCurrentSparkStreamingContext()
        setupStreamingContexts(engineContext, ssc)

        ssc
    }

    protected final def getCurrentSparkContext(): SparkContext = {
